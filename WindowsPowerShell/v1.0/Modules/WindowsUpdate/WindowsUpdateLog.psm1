...
function DecodeETL
{
    Param(
        ...
        [string] $SymbolServer = $(throw "'SymbolServer' parameter is required."),
    ...
    # symbol server
    $args.Add("-pdb")
    $args.Add("SRV*$SYM_CACHE*$SymbolServer")
...
function ConvertXmlToLog
{
    ...
    try
    {
        ...
        $xml.Events.Event | ForEach-Object {
            ...
            try
            {
                $debugNode = $_.DebugData

                if ($debugNode.Message.Length -gt 0)
                {
                    $systemNode = $_.System
                    ...
                    $writer.WriteLine("$($datetime.ToString($TIMESTAMP_FORMAT)) $($executionNode.ProcessID.ToString().PadRight(5)) $($executionNode.ThreadID.ToString().PadRight(5)) $($debugNode.FlagsName.ToString().PadRight(15)) $($debugNode.Message)")
                }
            }
            catch
            {
...
function ConvertCsvToLog
{
    ...
    try
    {
        ...
        $csv | ForEach-Object {
            ...
            try
            {
                if ($_.EventName -ne "EventTrace")
                {
                    [DateTime] $datetime = $_.TimeStamp
                    ...
                    # Log columns:
                    # Time ProcessID ThreadID Component Message
                    $writer.WriteLine("$($datetime.ToString($TIMESTAMP_FORMAT)) $($processId.ToString().PadRight(5)) $($threadId.ToString().PadRight(5)) $($_.Category.PadRight(15)) $($_.LogMessage)")
                }
            }
            catch
            {
...
function ConvertETLsToLog
{
    ...
    while ($processed -lt $etlList.Count)
    {
        ...
        {
            DecodeETL -ETLList $etlList.GetRange($processed, $MAX_ETL_PER_BATCH) -OutPath $numberedTempFile -Type $Type -SymbolServer $SymbolServer
            $processed += $MAX_ETL_PER_BATCH
        }
        else
        {
            DecodeETL -ETLList $etlList.GetRange($processed, $remaining) -OutPath $numberedTempFile -Type $Type -SymbolServer $SymbolServer
            $processed += $remaining
        }       
...
function Get-WindowsUpdateLog
{
    ...
    Param(
        ...
        [parameter(
            Position = 2)]
        [string] $SymbolServer = "http://msdl.microsoft.com/download/symbols",
    ...
    end
    {
        ...
        ConvertETLsToLog -ETLPaths $etls -LogPath $LogPath -Type $ProcessingType -SymbolServer $SymbolServer
    }
}
